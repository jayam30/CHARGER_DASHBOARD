...
//   const [targetSOC, setTargetSOC] = useState<number | null>(null); // State to store targetSOC from Firebase
//   // Timer in seconds (set to desired duration)
...
//   useEffect(() => {
//     const targetSOCRef = ref(database, "charging_status/targetSOC");

//     const unsubscribe = onValue(targetSOCRef, (snapshot) => {
//       const fetchedTargetSOC = snapshot.val();
//       setTargetSOC(fetchedTargetSOC);
//     });
...
//   useEffect(() => {
//     if (targetSOC !== null && SOC !== undefined) {
//       if (targetSOC <= SOC) {
//         setIsChargingInitialized(false);
//         set(ref(database, "charging_status/isChargingInitialized"), false); // Update in Firebase
//         console.log("TargetSOC reached or exceeded, stopping charging.");
//       }
//     }
//   }, [targetSOC, SOC]);
...
//     const status = stats;
//     console.log({status});
//     if (status?.isChargingInitialized && !isPaused) {
...
//       if (!bmsData.targetSOC || SOC >= bmsData.targetSOC!) return;
...
// export default SocChargingPage;
...
const [chargingDuration, setChargingDuration] = useState({ hours: 0, minutes: 0, seconds: 0 });
const [durationTimer, setDurationTimer] = useState<NodeJS.Timeout | null>(null);// State to store targetSOC from Firebase
  // Timer in seconds (set to desired duration)
  ...
  //     interval = setInterval(() => {
  //       let interval: NodeJS.Timeout;
  //      if (isChargingInitialized && !fodTriggered && !misalignmentTriggered && !emergencyStop) {
  //      interval = setInterval(() => {
  //     setChargingDuration(prev => {
  ...
  useEffect(() => {
    ...
    if (status?.isChargingInitialized && !isPaused && isChargingInitialized && 
        !fodTriggered && !misalignmentTriggered && !emergencyStop) {
      ...
      interval = setInterval(() => {
        ...
        // Calculate energy if current is flowing
        if (current > 0) {
          const calculatedPower = Number((voltage * current).toFixed(2));
          const powerInKW = calculatedPower / 1000;
          const calculatedEnergy = powerInKW / 3600;
...
// // Add this effect to reset the duration when charging stops
// useEffect(() => {
//   if (!isChargingInitialized || fodTriggered || misalignmentTriggered || emergencyStop) {
//     setChargingDuration({ hours: 0, minutes: 0, seconds: 0 });
//     if (durationTimer) {
//       clearInterval(durationTimer);
//     }
//   }
// }, [isChargingInitialized, fodTriggered, misalignmentTriggered, emergencyStop]);

// // Add this where you want to display the timer in your JSX
const formattedDuration = `${formatTime(chargingDuration.hours)}:${formatTime(chargingDuration.minutes)}:${formatTime(chargingDuration.seconds)}`;
      ...
      <div className="w-full px-12 mt-7">
        <div className="grid grid-cols-2 gap-6">
          ...
          >
            ...
            Counter Seconds:{" "}
            <span className="group-hover:text-cyan-400/90 transition-colors duration-300">
              {formatTime(chargingDuration.seconds)}
            </span>
          </motion.div>
