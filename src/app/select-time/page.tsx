...
//   const { voltage, current, SOC, loading } = useBMSData();
...
//     if (fodTriggered) {
//       toast.error("Foreign object detected on charging pad");
//       return;
//     }

//     if (misalignmentTriggered) {
//       toast.error("Vehicle is not properly aligned");
//       return;
//     }

//     if (emergencyStop) {
//       toast.error("Emergency stop is active");
//       return;
//     }

//     if (!isReceiverCoilDetected) {
//       toast.error("Receiver coil not detected. Please park properly.");
//       router.push("/park");
//       return;
//     }
...
//                         : "text-neutral-400 hover:text-white hover:border-neutral-300"
//                     }`}
//                   >
//                     {mins >= 60 ? `${Math.floor(mins / 60)}h` : `${mins}m`}
//                   </Button>
...
//     if (!isReceiverCoilDetected) {
//       toast.error("Vehicle not properly aligned");
//       router.push("/park");
//       return;
//     }
...
//   useEffect(() => {
//     // Redirect to charging page if charging is active
//     if (status?.isChargingInitialized) {
//       router.push("/charge");
//     } else if (status?.duration?.endTime) {
//       // If there's an endTime but charging isn't initialized, it means charging has ended
//       toast.info("Charging session completed");
//       router.push("/"); // or wherever you want to redirect after charging ends
//     }
//   }, [status?.isChargingInitialized, status?.duration?.endTime, router]);
...
import { useEffect, useState } from "react";
import ChargingPadWarning from "@/components/FodDialog";
import { ChevronLeft, ChevronRight, Timer, Zap, Info, Home } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { useChargingStatus } from "@/hooks/useChargingStatus";
import { useRouter } from "next/navigation";
...
export default function Page() {
 ...
 return (
   ...
   >
     <Button
          className="mb-6 text-lg bg-red-500 hover:bg-red-600 text-white font-semibold px-6 py-3 rounded-full shadow-lg transition-all duration-200"
